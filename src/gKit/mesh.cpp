...
vec3 getTangeant(const Vector &a, const Vector &b, const Vector &c, const Vector &ta, const Vector &tb, const Vector &tc) {
    Vector ab = b - a;
    Vector ac = c - a;

    Vector deltaUV1 = tb - ta;
    Vector deltaUV2 = tc - ta;
...
void Mesh::computeTangeant() {
    if(!has_normal() && !has_texcoord()) return;
    m_tangents.clear();
    m_tangents.resize(m_positions.size());
    if(m_indices.size()) {
        for(int i = 0; i < m_indices.size(); i+=3) {
            Vector a = Vector(m_positions[m_indices[i]]);
            Vector b = Vector(m_positions[m_indices[i+1]]);
            Vector c = Vector(m_positions[m_indices[i+2]]);

            Vector ta = Vector(m_texcoords[m_indices[i]]);
            Vector tb = Vector(m_texcoords[m_indices[i+1]]);
            Vector tc = Vector(m_texcoords[m_indices[i+2]]);

            Vector t = getTangeant(a, b, c, ta, tb, tc);
            m_tangents[m_indices[i]] = t;
            m_tangents[m_indices[i+1]] = t;
            m_tangents[m_indices[i+2]] = t;
        }
    } else {
        for(int i = 0; i < m_positions.size(); i+=3) {
            Vector a = Vector(m_positions[i]);
            Vector b = Vector(m_positions[i+1]);
            Vector c = Vector(m_positions[i+2]);

            Vector ta = Vector(m_texcoords[i]);
            Vector tb = Vector(m_texcoords[i+1]);
            Vector tc = Vector(m_texcoords[i+2]);
...
std::vector<TriangleGroup> Mesh::groups( const std::vector<unsigned int>& triangle_properties )
{
    ...
    if(m_indices.size())
    {
        ...
        for(unsigned i= 0; i < remap.size(); i++)
        {
            ...
            if(id != property_id)
            {
                TriangleGroup group= {property_id, first, int(3*i) - first};
                bounds(group.first/3, (group.first + group.n)/3, group.min, group.max);
        ...
        // dernier groupe
        TriangleGroup group= {property_id, first, int(3 * remap.size()) - first};
        bounds(group.first/3, (group.first + group.n)/3, group.min, group.max);
        groups.push_back(group);
...
void Mesh::bounds(int is, int ie, Point& pmin, Point& pmax ) const
{
    if(m_positions.size() < 1)
        return;
    auto tstart = this->triangle(is);
    pmin= Point(tstart.a);
    ...
    {
        auto tcurrent = this->triangle(i);

        pmin = Point(std::min(pmin.x, tcurrent.a.x), std::min(pmin.y, tcurrent.a.y), std::min(pmin.z, tcurrent.a.z));
        pmax = Point(std::max(pmax.x, tcurrent.a.x), std::max(pmax.y, tcurrent.a.y), std::max(pmax.z, tcurrent.a.z));

        pmin = Point(std::min(pmin.x, tcurrent.b.x), std::min(pmin.y, tcurrent.b.y), std::min(pmin.z, tcurrent.b.z));
        pmax = Point(std::max(pmax.x, tcurrent.b.x), std::max(pmax.y, tcurrent.b.y), std::max(pmax.z, tcurrent.b.z));

        pmin = Point(std::min(pmin.x, tcurrent.c.x), std::min(pmin.y, tcurrent.c.y), std::min(pmin.z, tcurrent.c.z));
        pmax = Point(std::max(pmax.x, tcurrent.c.x), std::max(pmax.y, tcurrent.c.y), std::max(pmax.z, tcurrent.c.z));
    }
...
GLuint Mesh::create_buffers( const bool use_texcoord, const bool use_normal, const bool use_color, const bool use_material_index )
{
    ...
    if(use_texcoord && has_texcoord() && has_position()){
        if(m_tangents.size() == 0) computeTangeant();
        m_vertex_buffer_size+= tangent_buffer_size();
    }
...
{
    // POSITION UVS NORMALS COLORS MATERIALS
    assert(m_vao > 0);
    assert(m_buffer > 0);
    if(!m_update_buffers)
        return 0;
    ...
    if(use_texcoord && has_texcoord() && has_position())
    {
        offset= offset + size;
        size= tangent_buffer_size();
        //~ glBufferSubData(GL_ARRAY_BUFFER, offset, size, tangent_buffer());   // copie les donnees dans le vertex buffer
        update.copy(GL_ARRAY_BUFFER, offset, size, tangent_buffer());           // copie les donnees dans le vertex buffer
        
        glVertexAttribPointer(5, 3, GL_FLOAT, GL_FALSE, 0, (const void *) offset);
...
void Mesh::draw( const int first, const int n, const GLuint program, const bool use_position, const bool use_texcoord, const bool use_normal, const bool use_color, const bool use_material_index )
{
    ...
    if(m_positions.size() != m_tangents.size() && has_texcoord() && has_position()) 
    {
        std::cout<<"TANGEANT SIZE MISMATCH"<<std::endl;
        computeTangeant();
    }
