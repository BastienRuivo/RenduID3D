    ...
    for(;;)
    {
        ...
        else if(line[0] == 'm')
        {
            if(sscanf(line, "map_Kd %[^\r\n]", tmp) == 1)
                ...
                material->ns_texture= materials.insert_texture( texture_filename(tmp, pathname(filename)).c_str() );
            else if(sscanf(line, "map_Bump %[^\r\n]", tmp) == 1)
                material->normal_texture = materials.insert_texture( texture_filename(tmp, pathname(filename)).c_str() );
